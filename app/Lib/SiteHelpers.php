	...
	public static function getMaxDayStatus($orderno, $status = "all"){
		if ($status =="all"){
			$someUsers = DB::table(DB::raw('inventory_history IH'))
					->select("status","last_update","remark")
			...
			$someUsers = DB::table(DB::raw('inventory_history IH'))
					->select("last_update","remark")
                    ->join(DB::raw("(SELECT order_no, max(id_history) id_history FROM `inventory_history` where status='".$status."' AND order_no='".$orderno."' group by order_no) b"), "IH.id_history","=","b.id_history")->first();
        }
		if (isset($someUsers)){			
			return $someUsers;
		}else{
...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function toView( $grids )
	{
		...
		foreach($grids as $grid)
		{
			...
			if($grid['detail'] =='1')  
			{
				...
				if($format_as =='image'){
					$val = "{!! SiteHelpers::showUploadedFile(\$row->".$grid['field'].",'$format_value' ) !!}";
	...
	public static function formatRows( $value ,$attr , $row = null )
	{
		...
		if($format_as =='image')
		{
			$value = self::showUploadedFile($value,$format_value );
		} elseif($format_as =='link') {
	...
	public static function getDistanceDay($status, $strdt1){
		if ($status=="in"){
	    	 $datetime1 = new \DateTime($strdt1);
